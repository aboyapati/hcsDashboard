			  ...
			  <c:if test="${not empty dashboardchartGraph}">
				 ...
				 <c:forEach var="graphObj" items="${dashboardchartGraph}" varStatus="rowCounter">
				  	<c:if test="${rowCounter.count % 3 == 1}">
				   	...
				   	<td>
					 	<a href="componentDashboard?componentId=${graphObj.id}" id="items">
				      		<div id="chartdiv${graphObj.id}" class="chartDiv"></div> 
				      	</a>
			      	</td>
...
$(document).ready(function() {
	...
	for(var i=0; i<graphObj.length;i++){
		...
		speedMeterChart(id,graphName,warn_start,warn_end,warn_end_max,step_value,dataVal);
		
	}
}); 

function speedMeterChart(id,graphName,warn_start,warn_end,warn_end_max,step_value,dataVal) {
	var chart = AmCharts.makeChart("chartdiv"+id, {"theme": "light",
		"type": "gauge",
		"axes": [{
		    "topTextFontSize": 20,
		    "topTextYOffset": 70,
